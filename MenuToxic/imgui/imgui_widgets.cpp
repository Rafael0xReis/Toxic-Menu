...
extern ImFont* TabIconFont;

bool ImGui::Tab(const char* label, const char* Icon, bool selected, float x)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    const ImRect bb(pos, pos + ImVec2(x, 50));
    ImGui::ItemSize(ImVec2(x, 50), style.FramePadding.y);
    ...
    ImVec2 TextCenter = ImVec2(pos.x + x / 2 - (CalcTextSize(Icon).x / 2),(pos.y + 50 / 2 - (CalcTextSize(Icon).y / 2)) - 5);

    PushFont(TabIconFont);

    if (!selected)
        draw->AddText(TextCenter, ImColor(255, 255, 255), Icon);
    else
        draw->AddText(TextCenter, ImColor(40, 140, 40), Icon);
